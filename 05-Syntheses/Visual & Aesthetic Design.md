...
- Playable characters: distinctive across all 8 player slots (Hot Potato)
...
- **Hot Potato context:** party game = cartoon aesthetic = limited, saturated palette with high contrast. Think *Mario Kart*, *Fall Guys*, *Brawlhalla*.
...
**The accent color principle:** in a limited palette, designate one high-saturation accent color for critical game elements (the Potato, the active player indicator, the winner's position). Everything else can be more muted; the accent color always draws the eye.
...
### Technical Considerations for UE5 (Hot Potato)

- **Lumen** (UE5 global illumination): dynamic real-time GI produces natural-feeling light. Good for realistic games; may need to be tuned for cartoon aesthetic
- **Material emissivity:** in a cartoon/party game context, character materials should have consistent lighting response to remain readable under varied environmental lighting
- **Player character readability:** ensure characters read clearly under all arena lighting conditions (avoid lighting that causes player silhouettes to blend with backgrounds)
...
For Hot Potato: exaggerated squash/stretch on the Potato character when they receive the role, when they tag, when they fail. This amplifies the comedy and makes every interaction feel impactful.
...
## 10. Hot Potato — Visual Design Implications

| Decision | Rationale |
|---|---|
| High-saturation cartoon palette | Party game tone; high visual energy matches social energy |
| 8 player colors with high contrast + distinct shapes | Observable play; colorblind accessibility |
| Single accent color (bright yellow or neon) for the Potato role | Always draws the eye; king-of-hill targeting requires knowing who's "it" |
| Exaggerated squash/stretch on tag impact | Cartoon physics; amplifies comedy of the tag moment |
| Anticipation animation on Potato about to tag | Telegraphed tell; fairness in competitive design |
| Bright, warm lighting in safe zones; harsher contrast in hot zones | Emotional guidance through space |
| Art bible from day 1 | Multiple artists will join; visual consistency requires documented rules |
