...
**Design for appropriate blame:** clear outcome attribution produces social moments. Unclear attribution ("who caused that?") produces frustration.
...
- **Single shared focal point** — all attention should converge on one thing at a time (the ball; the leader's position)
...
- **Server-authoritative physics** for shared objects — client-side physics prediction will desync under latency
