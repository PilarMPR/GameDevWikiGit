...
**For Hot Potato:** eight distinct player colors must work under colorblindness. Solution: shapes + patterns + labels on player characters in addition to colors. Each player has a unique combination of color AND geometric shape indicator.
...
**For Hot Potato:** party games need different difficulty management. Options:
- Potato scaling (adjusts to player skill dynamically)
...
**Hot Potato context:** the game targets Steam (global) and the party game demographic (all ages, mixed groups, diverse social circles). Character design should present a diverse roster in appearance; humor should be universal/physical rather than culturally specific; localization into key markets (Spanish, French, German, Simplified Chinese, Japanese, Brazilian Portuguese at minimum for Steam's primary audience markets) is a launch consideration.
...
## 10. Hot Potato — Accessibility Implications

| Category | Specific requirement |
|---|---|
| **Visual** | 8 player shapes + patterns (not just colors); colorblind palette mode; min 28pt text |
| **Auditory** | Visual companions for all audio cues; separate music/SFX/voice volume sliders |
| **Motor** | Full controller remapping; toggle for sustained inputs; no button mashing in core gameplay |
| **Cognitive** | Skill-based dynamic scaling (Potato gets stronger if falling behind); clear objective always visible |
| **Photosensitivity** | Photosensitive mode option; no sustained flash effects above 3Hz in default mode |
