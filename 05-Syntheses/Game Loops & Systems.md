...
These aren't exclusive — most games blend families. Hot Potato is primarily **physics** (parkour movement) + **social** (tag, trolling) with light **progression** (FAP accumulation).
...
## 11. Hot Potato — System Design Implications

| Decision | Rationale |
|---|---|
| Tag = balancing loop on running | The potato is a liability that creates a natural balancing force — no runaway leader possible |
| FAP = reinforcing loop on aggression | Trolling accrues points; aggression compounds itself — requires pacing via timer |
| Dynamic scaling on Potato (×1.2/×1.5) | Adds a balancing loop when chasing loop is failing — prevents turtling |
| Short match timer (est. 3–5 min) | Prevents reinforcing loops from running to completion; resets every session |
| 2-slot item system | Limits item snowballing; capacity constraint creates meaningful choice |
| Anti-camp mechanic | Balancing loop on passive play; discourages degenerate strategy |
