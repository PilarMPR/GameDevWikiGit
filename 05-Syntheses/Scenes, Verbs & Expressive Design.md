...
## 10. Hot Potato — Expressive Design Analysis

**Primary verbs:** run, jump, tag (transfer Potato), use item.
- All four are physical/action verbs. The game argues: physical skill and social disruption are the primary modes of interaction.

**Objects:** the Potato (liability), runners (targets for tag), items (tools), the arena (obstacle/opportunity).
- Objects frame the verbs: the Potato gives "tag" its meaning (tagging without the Potato role is meaningless). Items give "use" its meaning (different items = different meanings of using).

**Context:** cartoon aesthetic + children's game origin = playful, non-threatening. The frame teaches "this is chaotic fun, not serious competition."

**Expressive constraint:** the only path to winning requires social interaction (you can't win alone — you must tag others and affect the social space). The constraint argues: in this game, isolation is failure.

**Argument Hot Potato makes through its rules:**
- Passive play is bad (anti-camp mechanic)
- Social disruption is rewarding (FAP system)
- Everyone can be threatened equally (potato passes to anyone)
