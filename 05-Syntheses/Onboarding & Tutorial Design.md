...
1. **One-sentence rule** — the core mechanic must be communicable in a single sentence: "Everyone runs from the Potato; the person holding it when the timer ends loses."
...
## 11. Hot Potato — Onboarding Implications

| Decision | Rationale |
|---|---|
| One-sentence rule: "Don't be the Potato when the timer runs out" | Party game 90-second communicability requirement |
| Round 1: learn by dying — FAP scoring explained through natural play | Progressive disclosure; avoid pre-game tutorial wall |
| Button prompts visible at all times in early rounds | Just-in-time controls; no manual required |
| First 30 seconds: player can move, jump, and understand the game state | Core competence first; no item complexity until round 2 |
| FAP system revealed gradually: trolling actions are highlighted when performed | Discovery > explanation; the player learns by doing |
| Match summary shows what happened and why | Post-session reflection encodes lessons; spaced repetition at session scale |
| Skip tutorial option on second playthrough | Player autonomy; prevents friction for experienced players |
