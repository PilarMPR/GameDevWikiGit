...
## 10. Hot Potato — Information Design

| Information element | Current state | Design question |
|---|---|---|
| Who is the Potato? | Public (shown on all players + minimap) | Should it ever be hidden? No — observable play requires it. |
| FAP scores | Public (all players can see standings) | Public is correct — king-of-hill targeting requires visible leaders. |
| Item contents of opponent slots | Currently unspecified — should it be public or private? | Private creates counterplay depth; public creates chaotic "item targeting." |
| Timer remaining | Public | Always public — shared tension is the point. |
| Potato scaling state (has Tier 1 or 2 activated?) | Currently unspecified | Should be visible to all players — activating the target's escape window creates emergent behavior. |
